...
test('code', function (t, md) {
    ...
    t.equivalent(next, [mk_block('next')], 'next untouched when its not code');
    ...
    t.equivalent(next, [mk_block('  bar')], 'paragraph put back into next block');
...
test('bulletlist', function (t, md) {
    var bl = function () {
        return md.dialect.block.lists.apply(md, arguments);
    };
  ...
  */
   ...
   |foo
   */
    t.equivalent(bl(mk_block(' *\nfoo')), undefined, 'Space required after bullet to trigger list');
        ...
        [
            [
                ...
                [
                    'listitem',
                    'foo',
                    ['bulletlist', ['listitem', 'bar', ['bulletlist', ['listitem', 'baz']]]]
                ]
            ]
        ],
   ...
   */
    t.equivalent(
        ...
        [
            [
                'bulletlist',
                ['listitem', 'foo', ['bulletlist', ['listitem', 'bar']]],
                ['listitem', 'baz']
            ]
        ],
        ...
        [
            [
                ...
                [
                    'listitem',
                    'foo',
                    ['bulletlist', ['listitem', 'bar'], ['listitem', 'baz'], ['listitem', 'fnord']]
                ]
            ]
        ],
   ...
   */
    t.equivalent(
        ...
        [['bulletlist', ['listitem', ['para', 'foo']], ['listitem', ['para', 'bar']]]],
   ...
   */
    t.equivalent(
        ...
        [
            [
                'bulletlist',
                ['listitem', 'foo', ['bulletlist', ['listitem', 'bar'], ['listitem', 'baz']]]
            ]
        ],
        'Indenting Case V'
    );
                ...
                [
                    ...
                    [
                        ...
                        [
                            ...
                            [
                                'bulletlist',
                                ['listitem', 'three', ['bulletlist', ['listitem', 'four']]]
                            ]
                        ]
                    ]
                ]
...
test('inline_strong_em', function (t, md) {
    ...
    t.equivalent(
        md.processInline('*abc `code`'),
        ['*abc ', ['inlinecode', 'code']],
        'strong/em II'
    );
    t.equivalent(
        md.processInline('*abc**def* after'),
        [['em', 'abc**def'], ' after'],
        'strong/em III'
    );
    t.equivalent(
        md.processInline('*em **strong * wtf**'),
        [['em', 'em **strong '], ' wtf**'],
        'strong/em IV'
    );
...
test('inline_img', function (t, md) {
    t.equivalent(
        md.processInline('![alt] (url)'),
        [['img', {href: 'url', alt: 'alt'}]],
        'inline img I'
    );

    t.equivalent(
        md.processInline("![alt](url 'title')"),
        [['img', {href: 'url', alt: 'alt', title: 'title'}]],
        'inline img II'
    );

    t.equivalent(
        md.processInline("![alt] (url 'tit'le') after')"),
    ...
    t.equivalent(
        md.processInline('![Alt text](/path/to/img\\\\.jpg "Optional title")'),
        [['img', {href: '/path/to/img\\.jpg', alt: 'Alt text', title: 'Optional title'}]],
        'inline img IV'
    );

    t.equivalent(
        md.processInline('![alt][id]'),
...
test('inline_link', function (t, md) {
    t.equivalent(
        md.processInline('[text] (url)'),
        [['link', {href: 'url'}, 'text']],
        'inline link I'
    );

    t.equivalent(
        md.processInline("[text](url 'title')"),
        [['link', {href: 'url', title: 'title'}, 'text']],
        'inline link II'
    );

    t.equivalent(
        md.processInline("[text](url 'tit'le') after')"),
    ...
    t.equivalent(
        md.processInline('[to put it another way][SECTION 1] or even [link this](#SECTION-1)'),
        [
            [
                'link_ref',
...
test('inline_autolink', function (t, md) {
    t.equivalent(
        md.processInline('<http://foo.com>'),
        [['link', {href: 'http://foo.com'}, 'http://foo.com']],
        'autolink I'
    );

    t.equivalent(
        md.processInline('<mailto:foo@bar.com>'),
        [['link', {href: 'mailto:foo@bar.com'}, 'foo@bar.com']],
        'autolink II'
    );

    t.equivalent(
        md.processInline('<foo@bar.com>'),
        [['link', {href: 'mailto:foo@bar.com'}, 'foo@bar.com']],
        'autolink III'
    );
});
...
test('header_in_paragraph', function (t, md) {
    var tree = ['markdown', ['para', 'Foo'], ['header', {level: 1}, 'Title'], ['para', 'Bar']];
