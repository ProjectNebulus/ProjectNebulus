...
if (!(window.location.pathname === "/chat")) {
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            chat_sidebar.insertAdjacentHTML('beforeend', `<span id="notification_chat_sidebar" class="${parseInt(data) > 0 ? "" : "hidden"} overflow-hidden flex flex-grow left-5 absolute text-sm pl-2 pb-1 bg-red-500 border-4 border-white dark:border-gray-800 rounded-full w-5 h-5" style="padding-left:7px;padding-right:7px">${data}</span>`);
            ...
            socket.on('new_message', function (data) {
                ...
                notifyMe(`New message from ${data['author'][1]}`, `${data['content']}`, data['author'][2]);
